...
import { mapServiceToCategory, trackEvent } from '@/lib/gtag-helpers'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Track: cotizacion solicitada
        trackEvent('quote_request', {
          event_label: 'cotizacion_solicitada',
          service_type: formData.service || 'general',
          service_slug: formData.service || 'general',
          service_category: mapServiceToCategory(formData.service || 'general'),
          vehicle_brand: formData.vehicleBrand || 'unknown',
          vehicle_model: formData.vehicleModel || 'unknown',
          commune: formData.commune || 'unknown',
          cta_location: 'cotizar_form',
          source: 'cotizar_form',
        })
      } catch (quoteErr) {
      ...
      try {
        ...
        const bkId = (booking as any).bookingNumber || (booking as any).id
        setBookingNumber(bkId)

        // Track: booking creado (schedule)
        const totalCLP = (laborCost || 0) + (partsCost || 0) + (materialsCost || 0) + (travelCost || 0)
        trackEvent('schedule', {
          event_label: 'booking_created',
          service_type: formData.service || 'general',
          service_slug: formData.service || 'general',
          service_category: mapServiceToCategory(formData.service || 'general'),
          vehicle_brand: formData.vehicleBrand || 'unknown',
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                          {availableSlots.map((slot) => (
                            ...
                            >
                              {slot.time}
                              {!slot.available && (
                                <span className="block text-xs text-gray-500 font-normal">Ocupado</span>
                              )}
                            </button>
                          ))}
                        </div>
